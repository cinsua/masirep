...
export default function CajonesPage() {
  ...
  const fetchCajones = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/estanterias/${estanteriaId}/cajones`);
      const result = await response.json();
  ...
  const fetchOrganizadores = async () => {
    try {
      const response = await fetch(`/api/estanterias/${estanteriaId}/organizadores`);
      const result = await response.json();
  ...
  const handleOrganizadorClick = (organizador: Organizador) => {
    ...
    router.push(`/ubicaciones/${ubicacionId}/estanterias/${estanteriaId}/organizadores/${organizador.id}`);
  };
  ...
  const handleOrganizadorSubmit = async (data: OrganizadorFormData) => {
    try {
      ...
      const url = editingOrganizador
        ...
        : `/api/estanterias/${estanteriaId}/organizadores`;
          ...
          <div className="flex items-center space-x-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                <EntityIcon entityType="estanteria" className="h-6 w-6 text-gray-600" />
                {estanteria.nombre}
              </h1>
      ...
      <OrganizadorForm
        ...
        description={
          editingOrganizador
            ...
            : "Agrega un nuevo organizador a esta estantería con cajoncitos fijos"
        }
        parentType="estanteria"
        parentId={estanteriaId}
        isSubmitting={isSubmitting}
